...
import 'package:empty_aplicattion/data/people.dart';
...
class ListNamaView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Dafar Nama"),
      ),
      body: LayoutBuilder(builder: (context, constraints){
        if(constraints.maxWidth > 600){
          return const WideLayout();
        }else{
          return const NarrrowLayout();
        }
      }),
    );
  }
...
class NarrrowLayout extends StatelessWidget {
  const NarrrowLayout({super.key});
